...
export default async function ThreatIndexBar() {
  ...
  return (
    <div className="relative w-full border border-zinc-800 bg-[#080a0e] overflow-hidden my-8 shadow-2xl">
      ...
      {/* HEADER TÁTICO */}
      <div className="relative z-10 flex items-center gap-4 px-5 sm:px-7 py-3 border-b border-zinc-800 bg-[#0a0c10]">
        <span className="relative flex h-2 w-2 shrink-0">
          <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-600 opacity-75" />
          <span className="relative inline-flex rounded-full h-2 w-2 bg-red-600" />
        </span>
        <div className="flex flex-col sm:flex-row sm:items-center gap-0.5 sm:gap-3">
          <span className="text-[11px] font-black uppercase tracking-[0.3em] text-zinc-100 font-mono">
            Radar de Crises Global
          </span>
          <span className="hidden sm:block text-zinc-800">|</span>
          <span className="text-[10px] font-mono uppercase tracking-widest text-zinc-500">
            ReliefWeb Intelligence Center &bull; UN/OCHA
          </span>
        </div>
        ...
        <Link
          ...
          className="hidden sm:flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-zinc-400 hover:text-white transition-colors"
        >
          Análise Completa <span aria-hidden="true">→</span>
        </Link>
      </div>

      {/* GRID DE ZONAS DE RISCO */}
      <div className="relative z-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 divide-x divide-y lg:divide-y-0 divide-zinc-800/60 font-mono">
        {zones.map((zone) => (
          <div
            key={zone.id}
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div className="flex flex-col gap-0.5 min-w-0">
                <span className="text-[10px] font-black uppercase tracking-[0.15em] text-zinc-500 truncate">
                  {zone.region}
                </span>
                <span className="text-[9px] font-bold uppercase tracking-wider text-zinc-600 truncate">
                  {zone.type}
                </span>
              </div>
            ...
            <p className="text-[11px] font-black uppercase leading-tight text-zinc-200 line-clamp-2 min-h-[2.4em]">
              {zone.label}
            </p>
            ...
            <div className="mt-auto space-y-3">
              ...
              <div className="flex flex-col gap-0.5">
                <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: zone.color }}>
                  {zone.status}
                </span>
                {zone.since && (
      ...
      {/* FOOTER DA BARRA */}
      <div className="relative z-10 flex items-center justify-between gap-4 px-5 sm:px-7 py-2 border-t border-zinc-800/70 bg-[#050608]">
        <div className="flex items-center gap-3">
          <span className="text-[9px] font-mono uppercase tracking-[0.2em] text-zinc-600">
            Term. ID: VETOR-INTEL-01
          </span>
        ...
        <span className="text-[9px] font-black uppercase tracking-widest text-zinc-500">
           Sinal: Estável &bull; {zones.length} Alvos em Monitoramento
        </span>
      </div>
    </div>
  );
