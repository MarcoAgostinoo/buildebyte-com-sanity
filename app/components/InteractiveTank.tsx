...
export default function InteractiveTank() {
  ...
  const fireCannon = useCallback(() => {
    ...
    setProjectiles((prev) => [
      ...prev,
      { id: fireId, angle: angleRef.current, type: "cannon", dist, duration },
    ]);
    ...
    setTimeout(() => {
      setImpacts((prev) => [
        ...prev.slice(-30),
        { id: fireId, x: targetX, y: targetY, type: "cannon" },
      ]);
    ...
    setTimeout(() => {
      ...
      setParticles((prev) =>
        prev.filter((p) => p.id !== fireId + 1 && p.id !== fireId + 2),
      );
    }, impactTime + 1000);
  }, [isShootingCannon]);
  ...
  const startMG = useCallback(() => {
    ...
    const fireMGBullet = () => {
      ...
      setProjectiles((prev) => [
        ...prev,
        { id: fireId, angle: specificAngle, type: "mg", dist, duration },
      ]);
      ...
      setTimeout(() => {
        setImpacts((prev) => [
          ...prev.slice(-100),
          { id: fireId, x: targetX, y: targetY, type: "mg" },
        ]);
  ...
  return (
    <>
      <div className="relative w-36 h-40 flex items-center justify-center drop-shadow-2xl select-none overflow-visible z-10">
        {/* ESTEIRAS */}
        <div
          className={`absolute left-1 top-0 w-7 h-40 rounded-md border border-black shadow-[inset_0_0_10px_rgba(0,0,0,0.8)] transition-transform duration-75 ${isShootingCannon ? "-translate-y-1" : ""}`}
          style={{
            background:
              "repeating-linear-gradient(0deg, #1c1917, #1c1917 4px, #0c0a09 4px, #0c0a09 8px)",
          }}
        ></div>
        <div
          className={`absolute right-1 top-0 w-7 h-40 rounded-md border border-black shadow-[inset_0_0_10px_rgba(0,0,0,0.8)] transition-transform duration-75 ${isShootingCannon ? "translate-y-1" : ""}`}
          style={{
            background:
              "repeating-linear-gradient(0deg, #1c1917, #1c1917 4px, #0c0a09 4px, #0c0a09 8px)",
          }}
        ...
        <div className="absolute w-20 h-28 bg-[#3a4032] border-2 border-[#22261d] rounded-sm shadow-[inset_0_0_15px_rgba(0,0,0,0.9)] flex flex-col items-center">
          <div className="w-full h-8 border-b-2 border-black/40 shadow-inner rounded-t-sm flex items-start justify-center pt-1 relative">
            <div className="absolute left-1 top-1 w-1.5 h-1.5 rounded-full bg-yellow-200/50 shadow-[0_0_5px_#fef08a]"></div>
            <div className="absolute right-1 top-1 w-1.5 h-1.5 rounded-full bg-yellow-200/50 shadow-[0_0_5px_#fef08a]"></div>
            <div className="w-10 h-4 bg-[#2f3329] rounded-b-sm border-x border-b border-black/30"></div>
          </div>
          <div className="absolute bottom-1 w-12 h-6 flex flex-col justify-evenly bg-black/80 rounded-sm p-1 border border-white/5">
            <div className="w-full h-0.5 bg-zinc-700/50"></div>
            <div className="w-full h-0.5 bg-zinc-700/50"></div>
            <div className="w-full h-0.5 bg-zinc-700/50"></div>
            <div className="w-full h-0.5 bg-zinc-700/50"></div>
          </div>
        ...
        >
          {/* Canhão Principal */}
          <div
            className={`absolute left-1/2 top-1/2 -translate-y-1/2 flex items-center origin-left z-10 transition-transform ${isShootingCannon ? "duration-75 -translate-x-2" : "duration-500 translate-x-0 ease-out"}`}
          >
            ...
            <div className="w-4 h-3.5 bg-linear-to-b from-[#525252] to-[#171717] rounded-sm border border-black relative flex justify-center items-center">
              {isShootingCannon && (
                <div className="absolute left-full top-1/2 -translate-y-1/2 flex items-center pointer-events-none origin-left">
                  <div className="absolute w-12 h-12 bg-white blur-[2px] rounded-full mix-blend-overlay animate-tank-flash z-30"></div>
                  <div className="absolute w-16 h-16 -left-4 bg-linear-to-r from-yellow-300 to-orange-600 rounded-full blur-md mix-blend-screen animate-tank-fireball z-20"></div>
                  <div className="absolute w-8 h-16 border-r-4 border-white rounded-[50%] opacity-50 animate-tank-shockwave z-40"></div>
                </div>
              )}
              {particles
                .filter((p) => p.type === "smoke")
                .map((p) => (
                  <div
                    key={p.id}
                    className="absolute left-full w-6 h-6 bg-stone-500/30 rounded-full blur-xl animate-tank-smoke pointer-events-none"
                  ></div>
                ))}
            </div>
          </div>
          ...
          <div className="absolute top-4 left-[35px] flex items-center origin-left z-30">
            <div
              className={`flex items-center transition-transform ${isShootingMG ? "-translate-x-1" : ""}`}
            >
              ...
              <div className="w-7 h-0.5 bg-zinc-800 border-y border-black relative">
                {isShootingMG && (
                  <div className="absolute left-full top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none">
                    <div className="w-full h-full bg-yellow-200 rounded-full blur-[1px] animate-mg-flash mix-blend-screen"></div>
                  </div>
                )}
              </div>
            </div>
          ...
          <div
            className={`absolute w-16 h-16 bg-[#4a523e] border border-black rounded-xl shadow-[inset_0_0_20px_rgba(0,0,0,0.6),0_10px_15px_-3px_rgba(0,0,0,0.5)] flex items-center justify-center transition-transform z-20 ${isShootingCannon ? "duration-75 -translate-x-1" : "duration-300 ease-out"}`}
          >
            <div className="relative w-7 h-7 bg-[#2f3329] border-2 border-black rounded-full flex items-center justify-center shadow-inner">
              <div className="absolute top-1 right-1 w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse shadow-[0_0_8px_rgba(239,68,68,0.8)]"></div>
            </div>
            {particles
              .filter((p) => p.type === "casing")
              .map((p) => (
                <div
                  key={p.id}
                  className="absolute top-2 left-1/2 w-4 h-1.5 bg-yellow-600 rounded-sm border border-yellow-800 animate-shell-eject pointer-events-none"
                ></div>
              ))}
            {particles
              .filter((p) => p.type === "mg-casing")
              .map((p) => (
                <div
                  key={p.id}
                  className="absolute top-2.5 left-[45px] w-1.5 h-0.5 bg-yellow-400 rounded-sm animate-mg-shell-eject pointer-events-none"
                ></div>
              ))}
          </div>
        </div>
        ...
        {projectiles.map((proj) => (
          <div
            key={proj.id}
            className="absolute z-50 pointer-events-none top-1/2 left-1/2 origin-left"
            style={{ transform: `rotate(${proj.angle}deg)` }}
          >
            {proj.type === "cannon" ? (
              <div
          ...
          <div className="absolute top-0 left-0 w-full pointer-events-none z-[9999]">
            {impacts.map((impact) => (
              ...
              >
                {impact.type === "cannon" ? (
                  <>
                    {/* Fundo preto do buraco (reduzido para w-6 h-6) */}
                    <div className="absolute w-6 h-6 -ml-3 -mt-3 bg-zinc-950 rounded-full shadow-[inset_0_0_5px_black,0_0_3px_rgba(0,0,0,0.8)]"></div>

                    {/* Metal derretido ao redor da cratera (reduzido para w-6 h-6 com borda fina) */}
                    <div className="absolute w-6 h-6 -ml-3 -mt-3 rounded-full border-[1.5px] border-orange-500 mix-blend-screen animate-crater-glow"></div>

                    {/* Luz da explosão / Flash (muito mais sutil, brilho central pequeno) */}
                    <div className="absolute w-2 h-2 -ml-1 -mt-1 bg-white rounded-full shadow-[0_0_10px_5px_#fcd34d] animate-impact-flash mix-blend-overlay"></div>

                    {/* Nuvem de Fumaça (reduzida para w-8 h-8 e menos blur) */}
                    <div className="absolute w-8 h-8 -ml-4 -mt-4 bg-black/50 rounded-full blur-md animate-crater-smoke"></div>
                  </>
                ) : (
                  <>
